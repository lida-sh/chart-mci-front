<template>
  <div class="pr-[0.5rem] pl-4 pb-[0.2rem] min-h-screen">
    ...
    <div class="overflow-x-auto flex flex-col gap-4">
      <Form class="card p-8 flex flex-col sm:grid sm:grid-cols-6 gap-2 w-full rounded-box shadow-lg bg-white">
        <app-select-input name="architecture_id" :options="architectures!" :label="$t('architecture_id')"
          class="sm:col-span-3" @selectedItem="changeSelectItem"></app-select-input>
        <app-select-input name="directorate_id" :options="directorates!" :label="$t('directorate_id')" class="sm:col-span-3"
          @selectedItem="(directorate) => handleFilter({ directorate_id: directorate })"></app-select-input>
      ...
      <div v-if="!(data?.departments && data?.departments?.length > 0) && !pending" class="flex w-full">
        <div class="card w-full p-8 h-40 border rounded-xl shadow-lg bg-white flex items-center justify-start">
          اداره ای ثبت نشده است.
        </div>
      </div>
      <div v-else class="card p-8 gap-4 w-full rounded-box shadow-lg bg-white">
        <h3 class="text-sm font-bold text-gray-700 mb-5">لیست ادارات مخابرات ایران</h3>
        <div class="flex flex-col items-center justify-center w-full divide-y divide-gray-300">
          <div
            class="grid grid-cols-12 xl:grid-cols-18 gap-2 h-14 w-full bg-gray-200 text-xs 3xl:text-sm py-4 px-2 border border-white">
            ...
            <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">اداره کل والد</div>
            <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">معماری والد</div>
            <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">تعداد ردیف کارشناسی</div>
            <div class="col-span-1 2xl:col-span-1 flex items-center justify-center">وضعیت</div>
            <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">تعداد ردیف کارشناسی</div>
            <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">تعداد کارشناس </div>
            <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">تعداد کارشناس قراردادی</div>
            <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">تعداد کارشناس پیمانکاری</div>
            <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">تعداد پرسنل زیر کارشناسی </div>
          ...
          <div class="flex flex-col items-center justify-center w-full divide-y divide-gray-200"
            v-for="(item, index) in data?.departments">
            <div class="grid grid-cols-12 2xl:grid-cols-18 gap-2 h-auto w-full bg-white text-xs 2xl:text-sm py-4 px-2 ">
              ...
              <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">{{ (item.directorate !== null) ? item.directorate.title : "بدون اداره کل والد" }}</div>
              <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">{{ item.architecture.title }}</div>
              <div class="col-span-2 2xl:col-span-3 flex items-center justify-center">{{ item.evaluated_expert_positions_count }}</div>
              ...
              <div class="hidden 2xl:col-span-2 2xl:flex items-center justify-center">{{  }}</div>
              <div class="hidden 2xl:col-span-2 2xl:flex items-center justify-center">{{ item.old_permanent_experts_count }}</div>
              <div class="hidden 2xl:col-span-2 2xl:flex items-center justify-center">{{ item.old_contracting_experts_count }}</div>
              <div class="hidden 2xl:col-span-2 2xl:flex items-center justify-center">{{ item.old_below_expert_count }}</div>
              <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center">{{ item.user.fullName }}</div>
              <div class="hidden 2xl:col-span-1 2xl:flex items-center justify-center gap-1 2xl:gap-2">
                <NuxtLink :to="`sub-processes/${item.slug}`" class="text-indigo-700 text-xs hover:text-indigo-400">
                  ...
                  <button @click="deleteDepartmentConfirmation(item.id)"
            ...
            <div :id="'details' + item.id"
              ...
              <div class="flex items-center justify-between w-full py-3">
                <div class="flex items-center justify-center">تعداد ردیف کارشناسی</div>
                <div class="flex items-center justify-center">{{ item.evaluated_expert_positions_count }}</div>
              </div>
              <div class="flex items-center justify-between w-full py-3">
                <div class="flex items-center justify-center">تعداد کارشناس قراردادی</div>
                <div class="flex items-center justify-center">{{ item.old_permanent_experts_count }}</div>
              </div>
              <div class="flex items-center justify-between w-full py-3">
                <div class="flex items-center justify-center">تعداد کارشناس پیمانکاری</div>
                <div class="flex items-center justify-center">{{ item.old_contracting_experts_count }}</div>
              </div>
              <div class="flex items-center justify-between w-full py-3">
                <div class="flex items-center justify-center">تعداد پرسنل زیر کارشناسی</div>
                <div class="flex items-center justify-center">{{ item.old_below_expert_count }}</div>
              </div>
              <div class="flex items-center justify-between w-full py-3">
              ...
              <div class="flex items-center justify-between w-full py-2">
                ...
                <div class="flex items-center justify-center gap-2">
                  ...
                  <NuxtLink :to="`departments/edit/${item.id}`" class="text-amber-600 text-xs  hover:text-amber-400">
                    <icons-admin-edit></icons-admin-edit>
                  </NuxtLink>
                  <button @click="deleteDepartmentConfirmation(item.id)"
                    class="text-red-600 text-xs hover:text-red-400"><icons-admin-trash></icons-admin-trash></button>
    ...
    <app-modal v-model="deleteConfirmation">
      <template #title>
        <h3 class="text-sm xl:text-base font-bold text-blue-800">تایید حذف اداره</h3>
      </template>
      <div class="flex flex-col overflow-visible justify-center gap-10 mt-8">
        <h3 class="text-sm xl:text-base">آیا از حذف اداره مورد نظر مطمئن هستید؟</h3>
      </div>
      <template #actions>
        <div class="flex justify-between items-center w-full">
          <app-button :loading="loading"
            class="btn border-2 border-[#4749e3] text-[#4749e3] text-xs hover:bg-[#4749e3] hover:text-white hover:border-[#4749e3]"
            @click="deleteDepartmentconfirmed()">حذف
            اداره</app-button>
          <button
            class="btn border-2 border-[#4749e3] text-[#4749e3] text-xs hover:bg-[#4749e3] hover:text-white hover:border-[#4749e3]"
...
import { useGetBaseDirectoratesService, useGetDirectoratesService } from '~/composables/directorates/useDirectorate.service';
import { useDepartmentFilter } from '~/composables/departments/useDepartment.validation';
import { Form } from "vee-validate";
import { useGetDepartmentsService, useDeleteDepartmentService } from '~/composables/departments/useDepartment.service';
import type { DirectorateBaseDto } from '~/composables/directorates/directorate.dto';
const query = ref<any>({})
const { schema } = useDepartmentFilter()
...
const getDepartments = useGetDepartmentsService()
const { data, pending, error, refresh } = await useAsyncData('departments', () => getDepartments(query.value), { server: false })
...
const DepartmentIdForDelete = ref<number>(-1)
const deleteDepartmentConfirmation = (id: number) => {
  deleteConfirmation.value = true
  DepartmentIdForDelete.value = id
}
const deleteDepartment = useDeleteDepartmentService()
const deleteDepartmentconfirmed = () => {
  loading.value = true
  deleteDepartment(DepartmentIdForDelete.value).then((res) => {
    if (res !== undefined) {
      refresh()
      showToast({ message: "اداره مورد نظر حذف شد.", type: ToastEnum.success })
    }
  }).finally(() => {
    loading.value = false
