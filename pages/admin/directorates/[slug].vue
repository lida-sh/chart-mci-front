<template>
  ...
  <div v-else class="flex flex-col items-center justify-center px-[3rem] pb-[0.2rem]">
      <div class="bg-white shadow-lg rounded-2xl p-10 sm:p-[5rem] w-full min-h-screen flex flex-col gap-4">
          <h1 class="text-xl font-bold "> اداره کل {{ data?.title }} <span class="text-blue-700">{{ data?.title }}</span></h1>
          <div class="flex flex-col sm:flex-row justify-between items-center">
              <h6 class="text-base font-medium"> کد زیر فرایند:</h6>
...
import { useGetDirectorateBySlugService } from '~/composables/directorates/useDirectorate.service';
const route = useRoute()
const getDirectorate = useGetDirectorateBySlugService()
const { data, pending, error } = await useLazyAsyncData('directorate' + route.params.slug, () => getDirectorate(route.params.slug as string, {}), { server: false })
